...
The LATERAL subquery column "price_usd" conflicts with the function's
output column "price_usd". Alias all LATERAL subquery output columns to "eph_price".
*/
...
BEGIN
  ...
  WITH all_snapshots AS (
    SELECT
      ...
      eph.eph_price as snap_eth_usd,
      ps.is_interpolated as snap_interpolated
    FROM price_snapshots ps
    LEFT JOIN LATERAL (
      SELECT eth_price_history.price_usd AS eph_price FROM eth_price_history
      WHERE eth_price_history.timestamp <= ps.created_at
      ORDER BY eth_price_history.timestamp DESC
      LIMIT 1
    ) eph ON true
  ...
  price_24h AS (
    SELECT (ps24.price_eth * COALESCE(eph24.eph_price, ps24.eth_price_usd)) as price_usd_24h
    FROM price_snapshots ps24
    LEFT JOIN LATERAL (
      SELECT eth_price_history.price_usd AS eph_price FROM eth_price_history
      WHERE eth_price_history.timestamp <= ps24.created_at
      ORDER BY eth_price_history.timestamp DESC
      LIMIT 1
    ) eph24 ON true
  ...
  oldest_price AS (
    SELECT (pso.price_eth * COALESCE(epho.eph_price, pso.eth_price_usd)) as price_usd_oldest
    FROM price_snapshots pso
    LEFT JOIN LATERAL (
      SELECT eth_price_history.price_usd AS eph_price FROM eth_price_history
      WHERE eth_price_history.timestamp <= pso.created_at
      ORDER BY eth_price_history.timestamp DESC
      LIMIT 1
    ) epho ON true
