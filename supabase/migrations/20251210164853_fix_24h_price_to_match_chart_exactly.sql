/*
  # Fix 24h Price Selection to Match Chart Logic

  1. Problem
    - Price change function was using oldest price (>= 24h)
    - Chart function uses price closest to exactly 24h ago
    - This caused percentage mismatches for older tokens

  2. Solution
    - Change query to find first snapshot >= 24h ago (closest to 24h)
    - This matches the chart's get_price_chart_data_optimized logic
    - Both will now show identical percentages

  3. Changes
    - Updated prices_24h_ago CTE to use >= instead of <=
    - Ensures consistency across all price displays
*/
