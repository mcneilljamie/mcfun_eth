/*
  # Add Total Burned Value to Platform Statistics

  1. Changes
    - Add `total_burned_usd` column to platform_stats table
    - Update `update_platform_stats` function to calculate total burned value across all chains
    - Calculate dynamically based on current token prices and ETH price

  2. Calculation
    - For each token with burns: (burned_amount / 10^18) * token_price_eth * eth_price_usd
    - Sum across all tokens on all chains
*/

-- Add total_burned_usd column to platform_stats
ALTER TABLE platform_stats 
ADD COLUMN IF NOT EXISTS total_burned_usd numeric DEFAULT 0;

-- Update function to include total burned calculation
