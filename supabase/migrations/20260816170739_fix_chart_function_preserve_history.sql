/*
# Fix chart function to preserve all genuine trade points

## Problems Fixed
1. The chart function removed consecutive points with identical prices,
   discarding genuine trade records
2. The chart function used a time-based cutoff that didn't support
   true "ALL time" queries
3. The latest chart value and headline market cap used different sources

## Changes
1. Rewrite get_price_chart_data_optimized to:
   - NOT filter out consecutive duplicate prices
   - Support p_hours_back=0 or NULL to mean "all history from token launch"
   - Add a live market cap point using current reserves and current ETH price
   - Return the live market cap as last_price_usd so chart and headline agree
...
BEGIN
  -- NULL or 0 means all history from token launch
  ...
  SELECT price_usd INTO v_current_eth_price_usd
  FROM eth_price_history
  ORDER BY timestamp DESC
  LIMIT 1;

  -- Calculate live market cap from current reserves
  ...
  SELECT
    ...
    -- last_price_usd: use live market cap so chart and headline agree
