/*
  # Fix Aggregated Locks Column Reference

  1. Changes
    - Update get_aggregated_locks_by_token() to use correct column name
    - Change eth_price_usd to price_usd (correct column name in eth_price_history table)
    - Keep total_amount_locked as text to prevent JavaScript overflow

  2. Purpose
    - Fix SQL error preventing lock page from loading
*/
...
BEGIN
  ...
  SELECT
    ...
    COALESCE(
      (SUM(tl.amount_locked)::numeric / POWER(10, tl.token_decimals)) *
      (t.current_eth_reserve::numeric / NULLIF(t.current_token_reserve::numeric, 0)),
      0
    ) as total_value_eth,
    COALESCE(
