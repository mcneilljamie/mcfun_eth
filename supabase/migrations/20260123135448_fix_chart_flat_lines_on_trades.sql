/*
  # Fix Chart Flat Lines on New Trades

  1. Problem
    - When a new trade happens, the chart shows a flat horizontal line
    - This is because the compression logic keeps the "last point of flat period"
    - Then adds the new price point, creating a stair-step pattern
    
  2. Solution
    - Only keep points that represent actual price changes (>0.5%)
    - Keep the first point (for context)
    - Keep the very last point (current price)
    - Do NOT keep intermediate "bookend" points for flat periods
    
  3. Impact
    - Charts will show immediate price jumps without flat lines
    - More dynamic and exciting visual representation
    - Flat periods are still compressed (just not shown with bookends)
*/
...
BEGIN
  ...
  -- Only keep points that represent actual price changes or boundaries
  filtered_points AS (
    SELECT 
      a.*,
      CASE 
        ...
        -- Keep points with significant price change from previous (>0.5%)
        WHEN a.prev_price_eth IS NOT NULL 
          AND ABS((a.snap_price_eth - a.prev_price_eth) / NULLIF(a.prev_price_eth, 0)) > 0.005 THEN true
        ELSE false
      END as should_keep
