/*
  # Fix Price Metadata Calculation Range

  1. Problem
    - get_price_chart_data_optimized restricts ALL queries to p_hours_back window
    - This prevents calculating price_24h_ago_usd when p_hours_back is 24 hours
    - The 24h snapshot is outside the window and not accessible
    - Metadata fields return 0, causing "- 24h" to display with no percentage

  2. Solution
    - Keep chart data points within p_hours_back window (this is correct)
    - Allow metadata queries (price_24h, oldest_price) to access ALL snapshots
    - Remove v_cutoff_time restriction from metadata CTEs only
    - This ensures metadata is always calculated correctly regardless of time range

  3. Changes
    - Remove cutoff_time filter from price_24h CTE
    - Remove cutoff_time filter from oldest_price CTE
    - Keep cutoff_time filter for actual chart data points
*/
