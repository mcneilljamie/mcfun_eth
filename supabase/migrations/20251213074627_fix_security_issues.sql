/*
  ...
  1. Drop Unused Indexes
    - Drop `idx_tokens_block_number` - not being used
    - Drop `idx_tokens_block_hash` - not being used
    - Drop `idx_swaps_block_number` - not being used
    - Drop `idx_swaps_block_hash` - not being used
    - Drop `idx_price_snapshots_block_number` - not being used

  2. Fix Function Search Path
    - Recreate `update_platform_stats` function with immutable search_path
    - Set search_path explicitly to prevent security vulnerabilities

  3. Important Notes
    - These indexes were created for reorg protection but are not currently being used
    - The function security fix prevents search_path manipulation attacks
    - Auth DB Connection Strategy must be configured in Supabase Dashboard (cannot be fixed via SQL)
*/

-- Drop unused indexes
DROP INDEX IF EXISTS idx_tokens_block_number;
DROP INDEX IF EXISTS idx_tokens_block_hash;
DROP INDEX IF EXISTS idx_swaps_block_number;
DROP INDEX IF EXISTS idx_swaps_block_hash;
DROP INDEX IF EXISTS idx_price_snapshots_block_number;

-- Recreate update_platform_stats function with fixed search_path
