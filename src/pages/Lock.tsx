...
export function Lock({ onShowToast }: LockPageProps) {
  ...
  const loadAggregatedLocks = async () => {
    try {
      ...
      if (data) {
        ...
        // Filter out invalid entries and tokens with zero locked amount
        const validLocks = data.filter((lock: any) =>
          ...
          parseFloat(lock.total_amount_locked) > 0
        );
  ...
  const getStatusBadge = (status: 'active' | 'unlockable' | 'withdrawn') => {
    if (status === 'unlockable') {
      ...
      return (
        <span className="px-3 py-1 text-xs font-semibold rounded-full bg-gray-100 dark:bg-gray-800 text-gray-600 dark:text-gray-400 border border-gray-200 dark:border-gray-700">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {urlTokenAddress && !tokenStats ? (
          <div className="flex flex-col items-center justify-center py-20">
            ...
            <p className="text-gray-600 dark:text-gray-400 text-lg">{t('lock.loadingLocks')}</p>
          </div>
        ) : urlTokenAddress && tokenStats ? (
          <div className="mb-8">
            <button
          ...
          <div className="text-center mb-8">
            <h1 className="text-4xl font-bold text-gray-900 dark:text-white mb-4">{t('lock.title')}</h1>
        ...
        {urlTokenAddress && allLocks.length > 0 && (() => {
          ...
          if (activeLocks.length === 0) {
            return (
              <div className="mb-8 bg-white dark:bg-gray-800 rounded-xl shadow-lg p-12 text-center">
                <LockIcon className="w-16 h-16 text-gray-400 mx-auto mb-4" />
                <p className="text-gray-600 dark:text-gray-400 text-lg">{t('lock.noActiveLocksForToken')}</p>
              </div>
            );
          }
                  ...
                  >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
                      <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4">
                        <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">{t('lock.lockedOn')}</div>
                        <div className="text-sm font-semibold text-gray-900 dark:text-white">
                          {lockDate.toLocaleDateString()}
                        </div>
                      </div>
                      <div className="bg-gray-50 dark:bg-gray-900 rounded-lg p-4">
                        <div className="text-xs text-gray-500 dark:text-gray-400 mb-1">{t('lock.lockDuration')}</div>
                        <div className="text-sm font-semibold text-gray-900 dark:text-white">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6">
            ...
            {!account ? (
              <div className="text-center py-12">
                <LockIcon className="w-16 h-16 text-gray-400 mx-auto mb-4" />
                <p className="text-gray-600 dark:text-gray-400 mb-4">{t('lock.connectWallet')}</p>
              </div>
            ) : (
              <div className="space-y-4">
                  ...
                  {showTokenDropdown && popularTokens.length > 0 && (
                    <div className="absolute z-10 w-full mt-1 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-600 rounded-lg shadow-lg max-h-64 overflow-y-auto">
                      <div className="p-2 border-b border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900">
                        <div className="text-xs font-semibold text-gray-600 dark:text-gray-400 uppercase">{t('lock.popularTokensDropdown')}</div>
                      </div>
                        ...
                        >
                          <div className="flex items-center justify-between">
                            ...
                            <div className="ml-2 text-right">
                              ...
                              <div className="text-xs text-gray-500 dark:text-gray-400">{t('lock.marketCap')}</div>
                            </div>
                          </div>
                        </button>
                      ))}
                ...
                {tokenInfo && (
                  <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800/50 rounded-lg p-4">
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-gray-600 dark:text-gray-400">{t('lock.yourBalance')}:</span>
                      <span className="font-semibold text-gray-900 dark:text-white">
                ...
                <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-700/50 rounded-lg p-4">
                  <div className="flex items-start">
                    <AlertCircle className="w-5 h-5 text-yellow-600 dark:text-yellow-500 mr-2 mt-0.5" />
          ...
          <div className="bg-gradient-to-br from-purple-50 to-pink-50 dark:from-gray-800 dark:to-gray-900 rounded-xl shadow-lg p-6 border-2 border-purple-200 dark:border-gray-700">
            ...
            <div className="space-y-4">
              <div className="flex items-start">
                ...
                <div>
                  <h3 className="font-semibold text-gray-900 dark:text-white mb-1">{t('lock.step1Title')}</h3>
                  <p className="text-sm text-gray-700 dark:text-gray-300">{t('lock.step1Description')}</p>
                </div>
              </div>
              <div className="flex items-start">
                <div className="flex-shrink-0 w-8 h-8 bg-purple-600 text-white rounded-full flex items-center justify-center font-bold mr-3">
                ...
                <div>
                  <h3 className="font-semibold text-gray-900 dark:text-white mb-1">{t('lock.step2Title')}</h3>
                  <p className="text-sm text-gray-700 dark:text-gray-300">{t('lock.step2Description')}</p>
                </div>
              </div>
              <div className="flex items-start">
                <div className="flex-shrink-0 w-8 h-8 bg-purple-600 text-white rounded-full flex items-center justify-center font-bold mr-3">
                ...
                <div>
                  <h3 className="font-semibold text-gray-900 dark:text-white mb-1">{t('lock.step3Title')}</h3>
                  <p className="text-sm text-gray-700 dark:text-gray-300">{t('lock.step3Description')}</p>
                </div>
              </div>
            </div>
          </div>
              ...
              {topLockedTokens.map((aggLock, index) => {
                ...
                return (
                  <div
                    ...
                    className="border border-gray-200 dark:border-gray-700 rounded-lg p-4 hover:border-blue-400 hover:shadow-md transition-all cursor-pointer"
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3 flex-1">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1 flex-wrap">
                            <h3 className="text-lg font-bold text-gray-900 dark:text-white whitespace-nowrap">{aggLock.token_symbol}</h3>
                            <span className="text-sm text-gray-500 dark:text-gray-400 break-words">{aggLock.token_name || 'Unknown'}</span>
                          </div>
                          <div className="text-sm">
                            <span className="text-gray-600 dark:text-gray-400">{t('lock.totalLocked')}:</span>
                      ...
                      <div className="text-right ml-4">
                        <div className="text-sm text-gray-500 dark:text-gray-400 mb-1">{t('lock.totalValue')}</div>
